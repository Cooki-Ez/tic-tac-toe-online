...
export function UiModel({
  ...
  const handleClick = (e) => {
    const inModel = e.target.closest("[data-id=model]");
    if (inModel)
      return;
    onClose();
  }
  ...
  const model =
    ...
    >
      ...
      absolute top-0 left-[calc(100%_+_12px)]"
                onClick={onClose}
        >
  ...
  return createPortal(model, document.getElementById("models"));
}

UiModel.Header = function UiModelHeader({children, className}) {
  return <div className={clsx(className, "mx-6 pt-6 pb-4 text-2xl")}>{children}</div>
}

UiModel.Body = function UiModelBody({children, className}) {
  return <div className={clsx(className, "px-6")}>{children}</div>
}

UiModel.Footer = function UiModelFooter({children, className}) {
